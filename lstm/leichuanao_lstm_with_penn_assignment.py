...
class TextRNN(nn.Module):
    ...
    def forward(self, X):
        ...
        C_t = torch.ones([sample_size, n_hidden])
        for x in X:
            f_t = torch.sigmoid(self.W_fh(h_t) + self.W_fx(x) + self.b_f)
            i_t = torch.sigmoid(self.W_ih(h_t) + self.W_ix(x) + self.b_i)
            C_t_tem = self.tanh(self.W_cx(x) + self.W_ch(h_t) + self.b_c)
            C_t = f_t * C_t + i_t * C_t_tem
            o_t = torch.sigmoid(self.W_oh(h_t) + self.W_ox(x) + self.b_o)
            h_t = o_t * self.tanh(C_t)

        model_output = torch.softmax(self.W_qh(h_t) + self.b_q, dim = 0)
