...
class TextRNN(nn.Module):
    def __init__(self):
        ...
        self.b_a = nn.Parameter(torch.zeros([n_hidden]))
        '''
        ##Complete this code
        '''第一层'''
        #遗忘门
        self.W_fx1 = nn.Linear(emb_size, n_hidden, bias=False)
        self.W_fh1 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_f1 = nn.Parameter(torch.ones([n_hidden]))

        #输入门
        self.W_ix1 = nn.Linear(emb_size, n_hidden, bias=False)
        self.W_ih1 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_i1 = nn.Parameter(torch.ones([n_hidden]))

        self.W_cx1 = nn.Linear(emb_size, n_hidden, bias=False)
        self.W_ch1 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_c1 = nn.Parameter(torch.ones([n_hidden]))

        # self.C_t_before =
        # 输出门
        self.W_ox1 = nn.Linear(emb_size, n_hidden, bias=False)
        self.W_oh1 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_o1 = nn.Parameter(torch.ones([n_hidden]))

        # self.W_qh = nn.Linear(n_hidden, n_class, bias=False)
        # self.b_q = nn.Parameter(torch.ones([n_class]))

        '''第二层'''
        # 遗忘门
        self.W_fx2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W_fh2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_f2 = nn.Parameter(torch.ones([n_hidden]))

        # 输入门
        self.W_ix2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W_ih2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_i2 = nn.Parameter(torch.ones([n_hidden]))

        self.W_cx2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W_ch2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_c2 = nn.Parameter(torch.ones([n_hidden]))

        # self.C_t_before =
        # 输出门
        self.W_ox2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W_oh2 = nn.Linear(n_hidden, n_hidden, bias=False)
    ...
    def forward(self, X):
        ...
        model_output = torch.softmax(self.W(a_t) + self.b, dim = 0)
        '''
        # 初始化a_0为0
        ...
        for x in X:
            '''第一层'''
            f_t1 = torch.sigmoid(self.W_fh1(h_t1) + self.W_fx1(x) + self.b_f1)
            i_t1 = torch.sigmoid(self.W_ih1(h_t1) + self.W_ix1(x) + self.b_i1)
            C_t_tem1 = self.tanh(self.W_cx1(x) + self.W_ch1(h_t1) + self.b_c1)
            C_t1 = torch.mul(f_t1, C_t1) + torch.mul(i_t1, C_t_tem1)
            # C_t1 = f_t1 * C_t1 + i_t1 * C_t_tem1
            o_t1 = torch.sigmoid(self.W_oh1(h_t1) + self.W_ox1(x) + self.b_o1)
            # h_t1 = o_t1 * self.tanh(C_t1)
            h_t1 = torch.mul(o_t1, self.tanh(C_t1))
            '''
            print('矩阵o_t1的行:', o_t1.shape[0])
            print('矩阵o_t1的列:', o_t1.shape[1])
            print('矩阵h_t1的行:', h_t1.shape[0])
            print('矩阵h_t1的列:', h_t1.shape[1])
            '''

            # self.W_oh1(h_t2) + self.W_ox1(h_t1) + self.b_o2
            '''第二层'''
            f_t2 = torch.sigmoid(self.W_fh2(h_t2) + self.W_fx2(h_t1) + self.b_f2)
            i_t2 = torch.sigmoid(self.W_ih2(h_t2) + self.W_ix2(h_t1) + self.b_i2)
            C_t_tem2 = self.tanh(self.W_cx2(h_t1) + self.W_ch2(h_t2) + self.b_c2)
            C_t2 = f_t2 * C_t2 + i_t2 * C_t_tem2
            o_t2 = torch.sigmoid(self.W_oh2(h_t2) + self.W_ox2(h_t1) + self.b_o2)
            h_t2 = o_t2 * self.tanh(C_t2)


        model_output = torch.softmax(self.W_qh(h_t2) + self.b_q, dim = 0)
